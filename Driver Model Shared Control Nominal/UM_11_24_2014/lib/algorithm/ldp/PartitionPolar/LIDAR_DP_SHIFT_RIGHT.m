...
if DEBUG == 1 && CNT_2M > 1
    disp('LIDAR_DP_SHIFT_RIGHT, CNT_2M')
    pause
end

if isempty(IDX_2M)
    error('LIDAR_DP_SHIFT_RIGHT, IDX_2M not exist')
end

%%
% sector IDX_1i is shifted to the upper right 
% and is attached to sector IDX_2M
SECINFO_T2(IDX_2M, 6)       = IDX_1i; % <---
...
for k = IDX_2i + 1:IDX_2M - 1 % <---
    if SECINFO_T2(k, 3) == 0 || SECINFO_T2(k, 3) == -1
        IDX_2Mi_TYPE_T2     = 0;
    end
end

SECINFO_T2(IDX_2M, 7)       = IDX_2Mi_TYPE_T2; % <---
...
XX2                 = IPT.VP(1) + [0, DIST_LI]*cos(EP_POLAR(IDX_1i,1) + IPT.HA - pi/2); %<---
YY2                 = IPT.VP(2) + [0, DIST_LI]*sin(EP_POLAR(IDX_1i,1) + IPT.HA - pi/2); %<---
...
if isempty(XI1)
    error('LIDAR_DP_SHIFT_RIGHT, no intersection')
end
...
for k = 1:length(X_SEQ)
    ...
    if CONVEXITY == true
        AREA(k)     = triangle_area([X_SEQ(k), Y_SEQ(k)], [XI1, YI1], END_PTS(IDX_1i, 1:2));
    end
end
...
% else
    % error('LIDAR_DP_SHIFT_RIGHT, same line?')
% end

%%
if SIM.LOGS == YES
    fprintf(SIM.fid, '\t --------------------\n');
    formatSpec = '\t S%3.0f shift to right\n';
...
OLD_END_PTS                     = END_PTS(IDX_1i, 1:2);

END_PTS(IDX_1M, 1)              = XI1;
END_PTS(IDX_1M, 2)              = YI1;

END_PTS(IDX_1i, 1)              = XI2;
END_PTS(IDX_1i, 2)              = YI2;
END_PTS(IDX_1i, 3)              = XI1;
END_PTS(IDX_1i, 4)              = YI1;
