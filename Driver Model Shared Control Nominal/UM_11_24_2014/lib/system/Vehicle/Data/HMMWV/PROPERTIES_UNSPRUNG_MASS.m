function [MASS, X, Y, Z, IXX, IYY, IZZ] = PROPERTIES_UNSPRUNG_MASS()
    % Mass Information
...
MASS = MTireFL + (MSteering + MPowertrain + MSuspF + ...
    ...
    YTireFL = -909.701;
    ZTireFL = 415.417;
    LTireFL = [XTireFL, YTireFL, ZTireFL];
    
LUnsprungMass = (MTireFL*LTireFL + MSteering*LSteering/2/4 + ...
    MPowertrain*LPowertrain/2/4 + MSuspF*LSuspF/2/4 + ...
    MSuspFL*LSuspFL/2/4 + MSuspFR*LSuspFR/2/4 + ...
    MSuspR*LSuspR/2/4 + ...
    MSuspRL*LSuspRL/2/4 + MSuspRR*LSuspRR/2/4)/MASS;
X = LUnsprungMass(1);
Y = LUnsprungMass(2);
Z = LUnsprungMass(3);
...
IXXTireFL = 2.1635927621E+006;
IYYTireFL = 5.7507997882E+007;
IZZTireFL = 5.599502368E+007;
IXYTireFL = 0.0;
IXZTireFL = 7.8049908416E+006;
IYZTireFL = 0.0;
...
IXXTireFL = IXXTireFL - MTireFL*(YTireFL^2 + ZTireFL^2);
IYYTireFL = IYYTireFL - MTireFL*(XTireFL^2 + ZTireFL^2);
IZZTireFL = IZZTireFL - MTireFL*(XTireFL^2 + YTireFL^2);
IXYTireFL = IXYTireFL - MTireFL*XTireFL*YTireFL;
IXZTireFL = IXZTireFL - MTireFL*XTireFL*ZTireFL;
IYZTireFL = IYZTireFL - MTireFL*YTireFL*ZTireFL;
...
IXXSuspFL = IXXSuspFL + (MSuspFL/2/4)*((YSuspFL - Y)^2+(ZSuspFL - Z)^2);
IYYSuspFL = IYYSuspFL + (MSuspFL/2/4)*((XSuspFL - X)^2+(ZSuspFL - Z)^2);
IZZSuspFL = IZZSuspFL + (MSuspFL/2/4)*((XSuspFL - X)^2+(YSuspFL - Y)^2);
IXYSuspFL = IXYSuspFL + (MSuspFL/2/4)*(XSuspFL - X)*(YSuspFL - Y);
IXZSuspFL = IXZSuspFL + (MSuspFL/2/4)*(XSuspFL - X)*(ZSuspFL - Z);
IYZSuspFL = IYZSuspFL + (MSuspFL/2/4)*(YSuspFL - Y)*(ZSuspFL - Z);

IXXSuspFR = IXXSuspFR + (MSuspFR/2/4)*((YSuspFR - Y)^2+(ZSuspFR - Z)^2);
IYYSuspFR = IYYSuspFR + (MSuspFR/2/4)*((XSuspFR - X)^2+(ZSuspFR - Z)^2);
IZZSuspFR = IZZSuspFR + (MSuspFR/2/4)*((XSuspFR - X)^2+(YSuspFR - Y)^2);
IXYSuspFR = IXYSuspFR + (MSuspFR/2/4)*(XSuspFR - X)*(YSuspFR - Y);
IXZSuspFR = IXZSuspFR + (MSuspFR/2/4)*(XSuspFR - X)*(ZSuspFR - Z);
IYZSuspFR = IYZSuspFR + (MSuspFR/2/4)*(YSuspFR - Y)*(ZSuspFR - Z);

IXXSuspRL = IXXSuspRL + (MSuspRL/2/4)*((YSuspRL - Y)^2+(ZSuspRL - Z)^2);
IYYSuspRL = IYYSuspRL + (MSuspRL/2/4)*((XSuspRL - X)^2+(ZSuspRL - Z)^2);
IZZSuspRL = IZZSuspRL + (MSuspRL/2/4)*((XSuspRL - X)^2+(YSuspRL - Y)^2);
IXYSuspRL = IXYSuspRL + (MSuspRL/2/4)*(XSuspRL - X)*(YSuspRL - Y);
IXZSuspRL = IXZSuspRL + (MSuspRL/2/4)*(XSuspRL - X)*(ZSuspRL - Z);
IYZSuspRL = IYZSuspRL + (MSuspRL/2/4)*(YSuspRL - Y)*(ZSuspRL - Z);

IXXSuspRR = IXXSuspRR + (MSuspRR/2/4)*((YSuspRR - Y)^2+(ZSuspRR - Z)^2);
IYYSuspRR = IYYSuspRR + (MSuspRR/2/4)*((XSuspRR - X)^2+(ZSuspRR - Z)^2);
IZZSuspRR = IZZSuspRR + (MSuspRR/2/4)*((XSuspRR - X)^2+(YSuspRR - Y)^2);
IXYSuspRR = IXYSuspRR + (MSuspRR/2/4)*(XSuspRR - X)*(YSuspRR - Y);
IXZSuspRR = IXZSuspRR + (MSuspRR/2/4)*(XSuspRR - X)*(ZSuspRR - Z);
IYZSuspRR = IYZSuspRR + (MSuspRR/2/4)*(YSuspRR - Y)*(ZSuspRR - Z);

IXXSuspF = IXXSuspF + (MSuspF/2/4)*((YSuspF - Y)^2+(ZSuspF - Z)^2);
IYYSuspF = IYYSuspF + (MSuspF/2/4)*((XSuspF - X)^2+(ZSuspF - Z)^2);
IZZSuspF = IZZSuspF + (MSuspF/2/4)*((XSuspF - X)^2+(YSuspF - Y)^2);
IXYSuspF = IXYSuspF + (MSuspF/2/4)*(XSuspF - X)*(YSuspF - Y);
IXZSuspF = IXZSuspF + (MSuspF/2/4)*(XSuspF - X)*(ZSuspF - Z);
IYZSuspF = IYZSuspF + (MSuspF/2/4)*(YSuspF - Y)*(ZSuspF - Z);

IXXSuspR = IXXSuspR + (MSuspR/2/4)*((YSuspR - Y)^2+(ZSuspR - Z)^2);
IYYSuspR = IYYSuspR + (MSuspR/2/4)*((XSuspR - X)^2+(ZSuspR - Z)^2);
IZZSuspR = IZZSuspR + (MSuspR/2/4)*((XSuspR - X)^2+(YSuspR - Y)^2);
IXYSuspR = IXYSuspR + (MSuspR/2/4)*(XSuspR - X)*(YSuspR - Y);
IXZSuspR = IXZSuspR + (MSuspR/2/4)*(XSuspR - X)*(ZSuspR - Z);
IYZSuspR = IYZSuspR + (MSuspR/2/4)*(YSuspR - Y)*(ZSuspR - Z);

IXXSteering = IXXSteering + (MSteering/2/4)*((YSteering - Y)^2+(ZSteering - Z)^2);
IYYSteering = IYYSteering + (MSteering/2/4)*((XSteering - X)^2+(ZSteering - Z)^2);
IZZSteering = IZZSteering + (MSteering/2/4)*((XSteering - X)^2+(YSteering - Y)^2);
IXYSteering = IXYSteering + (MSteering/2/4)*(XSteering - X)*(YSteering - Y);
IXZSteering = IXZSteering + (MSteering/2/4)*(XSteering - X)*(ZSteering - Z);
IYZSteering = IYZSteering + (MSteering/2/4)*(YSteering - Y)*(ZSteering - Z);

IXXPowertrain = IXXPowertrain + (MPowertrain/2/4)*((YPowertrain - Y)^2+(ZPowertrain - Z)^2);
IYYPowertrain = IYYPowertrain + (MPowertrain/2/4)*((XPowertrain - X)^2+(ZPowertrain - Z)^2);
IZZPowertrain = IZZPowertrain + (MPowertrain/2/4)*((XPowertrain - X)^2+(YPowertrain - Y)^2);
IXYPowertrain = IXYPowertrain + (MPowertrain/2/4)*(XPowertrain - X)*(YPowertrain - Y);
IXZPowertrain = IXZPowertrain + (MPowertrain/2/4)*(XPowertrain - X)*(ZPowertrain - Z);
IYZPowertrain = IYZPowertrain + (MPowertrain/2/4)*(YPowertrain - Y)*(ZPowertrain - Z);

IXXTireFL = IXXTireFL + (MTireFL)*((YTireFL - Y)^2+(ZTireFL - Z)^2);
IYYTireFL = IYYTireFL + (MTireFL)*((XTireFL - X)^2+(ZTireFL - Z)^2);
IZZTireFL = IZZTireFL + (MTireFL)*((XTireFL - X)^2+(YTireFL - Y)^2);
IXYTireFL = IXYTireFL + (MTireFL)*(XTireFL - X)*(YTireFL - Y);
IXZTireFL = IXZTireFL + (MTireFL)*(XTireFL - X)*(ZTireFL - Z);
IYZTireFL = IYZTireFL + (MTireFL)*(YTireFL - Y)*(ZTireFL - Z);

IXX = (IXXTireFL + IXXSuspFL + IXXSuspFR + IXXSuspRL + IXXSuspRR + IXXSuspF + IXXSuspR + IXXSteering + IXXPowertrain)/1e6;
IYY = (IYYTireFL + IYYSuspFL + IYYSuspFR + IYYSuspRL + IYYSuspRR + IYYSuspF + IYYSuspR + IYYSteering + IYYPowertrain)/1e6;
IZZ = (IZZTireFL + IZZSuspFL + IZZSuspFR + IZZSuspRL + IZZSuspRR + IZZSuspF + IZZSuspR + IZZSteering + IZZPowertrain)/1e6;
IXY = (IXYTireFL + IXYSuspFL + IXYSuspFR + IXYSuspRL + IXYSuspRR + IXYSuspF + IXYSuspR + IXYSteering + IXYPowertrain)/1e6;
IXZ = (IXZTireFL + IXZSuspFL + IXZSuspFR + IXZSuspRL + IXZSuspRR + IXZSuspF + IXZSuspR + IXZSteering + IXZPowertrain)/1e6;
IYZ = (IYZTireFL + IYZSuspFL + IYZSuspFR + IYZSuspRL + IYZSuspRR + IYZSuspF + IYZSuspR + IYZSteering + IYZPowertrain)/1e6;
