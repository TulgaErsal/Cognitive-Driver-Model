...
g               = DATA_HMMWV.g;
MU              = DATA_HMMWV.MU;        %% unspring mass
MS              = DATA_HMMWV.MS;        %% sprung mass
VEHICLEMASS     = DATA_HMMWV.VEHICLEMASS;
LA              = DATA_HMMWV.LA;        %% front axle to cg
LB              = DATA_HMMWV.LB;        %% rear axle to cg
LC              = DATA_HMMWV.LC;        %% track width
LC              = LC/2*[1, 1, 1, 1];
IXX             = DATA_HMMWV.IXX;       %% roll inertia
IYY             = DATA_HMMWV.IYY;       %% pitch
IZZ             = DATA_HMMWV.IZZ;       %% yaw
KS              = DATA_HMMWV.KS;        %% suspension spring stiffness 
KT              = DATA_HMMWV.KT;        %% tire stiffness
CS              = DATA_HMMWV.CS;        
...
AREA_FRONTAL    = DATA_HMMWV.AREA_FRONTAL;
AIR_DENSITY     = DATA_HMMWV.AIR_DENSITY;
DRAG_COEFF      = DATA_HMMWV.DRAG_COEFF;

KAPPAMAX        = 0.5;                      % limits the longitudinal slip
DRIVE           = [0.25 0.25 0.25 0.25];    % apportionment of drive torque
BRAKE           = [0.2 0.2 0.3 0.3];        % apportionment of brake torque
...
% -------------Temporary offset for negative initialization-------------
% STATES_INIT(1)       = -9.61;
% STATES_INIT(2)       = -2.59;
% STATES_INIT(1)       = -100;
% STATES_INIT(2)       = -98.4901;
% -------------------------------end of offset ---------------------------
...
%************************************************************************%    
    ...
    %% Velocity at the Four Strut Mounting Points in Coordinate Frame 1 (1)
    ...
    RT      = (RW + XT)/(cos(THETA)*cos(PHI));              %% (15,XT<0 tire radial compression)
    % Instantaneous Height of the Four Strut Mounting Points
    LS      = (ZS_INIT - ZT_INIT) + (XS - XS_INIT);         %% (17,18)
    ...
    %% Velocity of Unsprung Masses in Coordinate Frame 1 (9,10)
    UU1     = US1 - LS*OMEGA_Y;
    VU1     = VS1 + LS*OMEGA_X;
    UU1dot  = US1dot - (LS*sys(11) + sys(21:24)'*OMEGA_Y);
    VU1dot  = VS1dot + (LS*sys(10) + sys(21:24)'*OMEGA_X);
    
    %% Longitudinal and Lateral Velocities of the Tire Contact Patch in Frame 2 (7,8)
    UG1     = UU1 - RT*OMEGA_Y;
    VG1     = VU1 + RT*OMEGA_X;
    WG1     = WU1;
...
%     FD(1)   = interp1(CS.RS, CS.FDF, DAMPER_VEL(1), 'pchip');     
%     FD(2)   = interp1(CS.RS, CS.FDF, DAMPER_VEL(2), 'pchip');     
%     FD(3)   = interp1(CS.RS, CS.FDR, DAMPER_VEL(3), 'pchip');
%     FD(4)   = interp1(CS.RS, CS.FDR, DAMPER_VEL(4), 'pchip');
    ...
    DAMPER_VEL = WS1 - WU1;
    for idx = 1:4
        if abs(WS1(idx)-WU1(idx))> CS.RS(end)
            DAMPER_VEL(idx) = CS.RS(end)*sign(WS1(idx)-WU1(idx));
            % disp(['# ', num2str(idx),' large damping velocity!!'])
        end
    end
    ...
    %% Tire veclocity along the tire direction
    ...
    for idx2 = 1:4
        ...
        if FTZ(idx2) == 0
            ...
            if abs(KAPPA(idx2)) > KAPPAMAX
                KAPPA(idx2) = sign(KAPPA(idx2))*KAPPAMAX;
            end
    ...
    %% Forces acting at the tire contact patch in coordinate frame 2 (3,4)
    FT      = [FTX;FTY;FTZ];
    ...
    %% Forces at the tire contact patch in coordinate frame 1 (2)
    FSG     = R21*FT;
    FSGX    = FSG(1,:);
    FSGY    = FSG(2,:);
    FSGZ    = FSG(3,:);

    %% Forces transmitted to sprung mass in coordinate frame 1 (21,22,23)
    ...
    %% Aerodynamics
    FXA     = -0.5*AREA_FRONTAL*AIR_DENSITY*DRAG_COEFF*abs(UG)*UG;
    FYA     = 0;
    FZA     = 0;
    MXA     = 0;
    MYA     = 0; 
...
%    FORCE       = [FTX,FTY,FTZ,MTZ,FSX,FSY,FSZ]';
    FORCE       = [FTX,FTY,FTZ,MTZ,FSX,FSY,FSZ,FXA]';
    
    SUSP        = [XT,XS,RT,LS,WU1,WS1,FD,FS,FJ]';

%    TIRE        = [UT,VT,WHEEL_SPEED,TW]';
    TIRE        = [UT,VT,WHEEL_SPEED,TS,TB,TW]';
    
%    COMMAND     = [SAFL,SAFR,SARL,SARR,AC]';
    COMMAND     = [SAFL,SAFR,SARL,SARR,T_shaft,B_shaft]';
